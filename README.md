...
- 수동으로 하다보면 실수로 인해 제대로 배포가 진행되지 않는 경우도 있었으며, 배포가 진행될 때마다 애플리케이션이 중단되어야만 했습니다.
- 수동으로 배포하는 문제점과 배포때마다 서버가 중단되는 문제점을 해결하고자, AWS Pipeline와 Nginx 을 도입했습니다.
- AWS Pipeline은 Github 소스 - CodeBuild - CodeDeploy로 구성되며 새롭게 추가된 코드를 자동으로 EC2 인스턴스에 배포할 수 있도록 해주는 CI/CD 파이프라인입니다.
- Nginx를 사용하여 현재 사용하고 있는 애플리케이션과 IDLE(휴식) 애플리케이션을 교차 사용해가며 Nginx Reload를 통해 서버가 무중단되고 배포될 수 있도록 했습니다.
