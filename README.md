...
- 아래 이미지는 애플리케이션에 임의의 부하를 가했을 때, Master/Slave DB 커넥션 개수를 비교한 그래프입니다. 커넥션의 개수가 분산됨 확인할 수 있었고 데이터베이스의 부하로 인한 병목을 줄일 수 있을 것으로 기대할 수 있었습니다.
...
- 한계점을 극복하고자 Connection Pool을 고려하여 적절한 커넥션 풀 개수를 조정해주면 성능에 좀 더 좋은 영향을 줄 수 있을 것이라 생각합니다.
